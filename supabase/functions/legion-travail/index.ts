...
- Tu n'as que des outils de LECTURE: la base de la place de marché (les vérifications ci-dessous) et, s'il y en a une plus bas, UNE recherche sur Internet faite pour ta tâche (sans elle, tu n'as pas navigué). Le code, tu le lis seulement si les vérifications ci-dessous montrent code_chercher / code_lire (le dépôt que l'entreprise a branché) : alors cite le fichier lu. Tu n'as pas accès aux e-mails, et tu ne peux rien modifier ni envoyer. Tu ne prétends donc JAMAIS avoir fait, changé, envoyé ou publié quoi que ce soit.
...
"statut": "termine" si tu as pu livrer; un plan, une liste d'étapes ou un renvoi du travail à Claude, à un collègue ou au fondateur n'est PAS un livrable quand tu peux faire la tâche toi-même (le texte, le calcul, la vérification) : fais-la et rends le résultat ; "bloque" si la tâche demande quelque chose que tu n'as pas (un accès, une décision, un outil d'écriture) — dans ce cas "livrable" contient ce que tu as quand même pu faire.
...
// `urgences` (Beau, 25/09 : « ils ne doivent pas être bloqués ») : le passage
// du quart d'heure. Seuls les agents qui ont une tâche urgente ou haute en
// attente travaillent ; ni mémoire des salons, ni plans, ni initiative.
async function travailler(service: Service, apiKey: string, entrepriseId: string, journal: string[], debut: number, urgences = false): Promise<boolean> {
  ...
  for (const c of (urgences ? [] : canaux) as Canal[]) {
  ...
  const directeurs = (urgences ? [] : machines).filter((a) => a.est_directeur && a.departement).sort((a, b) => Number(estDirection(a)) - Number(estDirection(b)));
  ...
  for (let i = 0; i < tranche.length; i += lot) {
    ...
    await Promise.all(tranche.slice(i, i + lot).map((a) => aPart(async () => {
      ...
      // Chaque prise compte un essai : le passage du quart d'heure laisse de
