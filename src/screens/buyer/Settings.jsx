      ...
      <div className="space-y-6 p-4">
        ...
        <section className="space-y-2">
          {/* Revoir la présentation.
              Beau: « s'il a déjà skip ça une fois, il ne doit plus le revoir —
              ou bien il y a un petit bouton au profil ». Les deux: on ne la
              repropose jamais d'elle-même, et qui veut la relire la trouve ici.
              On efface la marque puis on revient à l'accueil, où la
              présentation se rouvre d'elle-même — aucun paramètre d'adresse,
              donc rien qui puisse se rejouer en boucle au rechargement. */}
          <button
            onClick={() => {
              try { localStorage.removeItem('finjaro:intro-seen'); } catch { /* noop */ }
              navigate('/');
            }}
            className="flex w-full items-center justify-between rounded-card border border-hairline p-3 text-left text-body text-ink"
          >
            <span className="flex items-center gap-2"><IconSparkles size={18} /> {t('intro.replayIntro')}</span>
