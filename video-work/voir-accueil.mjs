// L'accueil de Legion: le plan unique, les connecteurs, la dépense. Reprend la mise en place de voir-action.
// dessus du clavier, en-tête en place), pas glisser vers le haut.
// On simule le clavier en réduisant la hauteur visible de 844 à 480 px.
...
async function page(largeur, hauteur = 900) {
  const ctx = await b.newContext({ viewport: { width: largeur, height: hauteur }, locale: 'fr-FR', hasTouch: true, isMobile: largeur < 800 });
...
const PLANS = [
  { id: 'p1', entreprise_id: EID, departement: 'Direction', horizon: 'semaine', created_at: il_y_a(300), contenu: "24 visiteurs engagés, 0 commande.\n\n## D'où on part\n- 391 articles, 65 boutiques\n- 6 ajouts au panier, 0 commande\n\n## Objectif de la semaine\n3 commandes payées.\n\n## Actions\n- [Alpha] Proposition recherche — demain\n- [Lien] Relancer les 4 vendeuses — mercredi\n\n## Ce qu'il faut au fondateur\nValider la fiche « commande sans compte »." },
  { id: 'p2', entreprise_id: EID, departement: 'Direction', horizon: 'mois', created_at: il_y_a(300), contenu: "## Objectif du mois\n20 commandes par semaine." },
  { id: 'p3', entreprise_id: EID, departement: 'Marketing', horizon: 'semaine', created_at: il_y_a(320), contenu: "## Objectif\nFaire venir 50 visiteurs engagés.\n- [Écho] Trois visuels" },
];
{
  const { ctx, pg } = await page(390, 844);
  await pg.route('**/rest/v1/legion_plans*', (r) => r.fulfill({ status: 200, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: JSON.stringify(PLANS) }));
  await pg.route('**/rest/v1/legion_connecteurs*', (r) => r.fulfill({ status: 200, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: JSON.stringify([{ type: 'finjaro-mesures', actif: true, config: {} }]) }));
  await pg.route('**/rest/v1/shops*', (r) => r.fulfill({ status: 200, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: JSON.stringify([{ id: 's1', name: 'Beauty hairs' }]) }));
  await pg.route('**/rest/v1/rpc/legion_depense_mois*', (r) => r.fulfill({ status: 200, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: JSON.stringify({ total_eur: 0.42, plafond_eur: 5, appels: 31, par_fonction: { legion_repondre: 0.3, legion_travail: 0.12 } }) }));
  await pg.goto(`${BASE}/legion/${EID}`, { waitUntil: 'domcontentloaded' }); await pg.waitForTimeout(3500); await fermerBandeaux(pg);
  const plan = pg.locator('text=Le plan de Finjaro').first();
  await plan.scrollIntoViewIfNeeded(); await pg.waitForTimeout(300);
  await pg.screenshot({ path: `${SORTIE}/legion-accueil-plan.png` });
  const conn = pg.locator('text=Ce que les agents peuvent lire').first();
  await conn.scrollIntoViewIfNeeded(); await pg.waitForTimeout(300);
  await pg.screenshot({ path: `${SORTIE}/legion-accueil-connecteurs.png` });
