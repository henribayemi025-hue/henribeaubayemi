// La fiche d'un agent: toucher le visage l'agrandit, ne le change pas. Reprend la mise en place de voir-clavier.
...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  await pg.locator('button[title="Fiche agent"]').first().click(); await pg.waitForTimeout(500);
  await pg.locator('.legion-modale button[title="Alpha"]').first().click(); await pg.waitForTimeout(400);
  await pg.screenshot({ path: `${SORTIE}/legion-fiche-grand.png` });
  console.log('image en grand:', await pg.locator('img[alt="Alpha"]').count());
  await ctx.close();
}
await b.close();
