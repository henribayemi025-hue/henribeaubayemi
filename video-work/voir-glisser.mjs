// Glisser un message pour y répondre (téléphone), et la barre au survol (ordinateur).
...
// Glisser la bulle de Claudinette vers la droite, au doigt.
async function glisser(pg, texte, dx) {
  const bulle = pg.locator(`p:has-text("${texte}")`).first();
  ...
  const point = (px) => [{ x: px, y, id: 1 }];
  await cdp.send('Input.dispatchTouchEvent', { type: 'touchStart', touchPoints: point(x) });
  for (let i = 1; i <= 8; i += 1) { await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: point(x + (dx * i) / 8) }); await pg.waitForTimeout(16); }
  await pg.screenshot({ path: `${SORTIE}/legion-glisse-pendant.png` });
...
}
{
  ...
  await pg.locator('p:has-text("Compris. Je regarde")').first().hover(); await pg.waitForTimeout(300);
  await pg.screenshot({ path: `${SORTIE}/legion-survol.png` });
  await ctx.close();
}
await b.close();
