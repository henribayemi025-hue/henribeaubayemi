// Une action proposée par un agent: la carte « Confirmer / Refuser ». Reprend la mise en place de voir-clavier.
...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  const avecAction = [...MESSAGES, { id: 'mx', entreprise_id: EID, canal_id: 'c0', auteur_id: 'a1', user_id: null, texte: 'Je te propose d’éteindre Plume pour l’instant : confirme et c’est fait.', genre: 'proposition', created_at: new Date().toISOString(), repondu_le: null, assigne_a: null, termine_le: null,
    meta: { par_ia: true, action: { type: 'eteindre_agent', agent_id: 'a5', agent: 'Plume', valeur: '', statut: 'a_confirmer' } } }];
  await pg.route('**/rest/v1/legion_messages*', (r) => r.request().method() === 'GET'
    ? r.fulfill({ status: 200, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: JSON.stringify([...avecAction].reverse()) }) : r.fallback());
  await pg.goto(`${BASE}/legion/${EID}`, { waitUntil: 'domcontentloaded' }); await pg.waitForTimeout(3500); await fermerBandeaux(pg);
  await pg.locator('nav button:has-text("Discussion")').first().click(); await pg.waitForTimeout(700);
  console.log('bouton Confirmer:', await pg.locator('button:has-text("Confirmer")').count());
  await pg.screenshot({ path: `${SORTIE}/legion-action.png` });
  await ctx.close();
}
await b.close();
