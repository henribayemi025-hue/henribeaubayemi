// Qui est dans un salon: ajouter un agent (téléphone). Reprend la mise en place de voir-clavier.
...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  await pg.locator('button[title="Qui est dans ce salon"]').first().click(); await pg.waitForTimeout(400);
  console.log('boutons Ajouter:', await pg.locator('button:has-text("Ajouter")').count());
  await pg.screenshot({ path: `${SORTIE}/legion-membres.png` });
  await ctx.close();
}
await b.close();
