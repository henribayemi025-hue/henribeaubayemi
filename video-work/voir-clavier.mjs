...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  await pg.locator('textarea').first().click();
  await pg.setViewportSize({ width: 390, height: 480 }); await pg.waitForTimeout(500);
  const boite = async (sel) => (await pg.locator(sel).first().boundingBox());
  console.log('en-tête', await boite('h3'), 'saisie', await boite('textarea'), 'scrollY', await pg.evaluate(() => window.scrollY), '--legion-h', await pg.evaluate(() => getComputedStyle(document.documentElement).getPropertyValue('--legion-h')));
  await pg.screenshot({ path: `${SORTIE}/legion-clavier.png` });
  console.log('zoom (taille de police de la saisie):', await pg.locator('textarea').first().evaluate((el) => getComputedStyle(el).fontSize));
  await ctx.close();
}
await b.close();
