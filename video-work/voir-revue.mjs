// La revue d'un livrable sur le tableau des tâches: « Valider » / « Renvoyer » avec remarque. Reprend la mise en place de voir-action.
...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  await pg.locator('nav button:has-text("Tâches")').first().click(); await pg.waitForTimeout(700);
  console.log('Valider:', await pg.locator('button:has-text("Valider")').count(), 'Renvoyer:', await pg.locator('button:has-text("Renvoyer")').count());
  const carte = pg.locator('button:has-text("Renvoyer")').first();
