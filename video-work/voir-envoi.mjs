// La case se vide dès l'envoi, même quand l'agent met du temps à répondre.
// Reprend la mise en place de voir-clavier.
// Le clavier du téléphone: la page doit rétrécir (zone de saisie juste au-
...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  await pg.route('**/functions/v1/**', async (r) => { await new Promise((ok) => setTimeout(ok, 4000)); r.fulfill({ status: 200, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: '{"messages":[]}' }); });
  await pg.route('**/rest/v1/legion_messages*', async (r) => {
    if (r.request().method() === 'POST') return r.fulfill({ status: 201, contentType: 'application/json', headers: { 'access-control-allow-origin': '*' }, body: JSON.stringify({ id: 'n1', entreprise_id: EID, canal_id: 'c0', auteur_id: 'a0', user_id: MOI, texte: 'bonjour', genre: 'info', meta: null, created_at: new Date().toISOString() }) });
    return r.fallback();
  });
  ...
  const zone = pg.locator('textarea').first();
  await zone.fill('bonjour'); await pg.locator('form button[type=submit]').first().click(); await pg.waitForTimeout(400);
  console.log('case apres envoi (agent encore en train de repondre):', JSON.stringify(await zone.inputValue()));
  await zone.fill('deuxieme message en cours'); await pg.waitForTimeout(4500);
  console.log('ce que je tapais pendant ce temps, apres la reponse:', JSON.stringify(await zone.inputValue()));
  await ctx.close();
}
await b.close();
