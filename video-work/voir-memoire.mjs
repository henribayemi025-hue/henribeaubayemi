// La mémoire sur l'accueil (téléphone). Reprend la mise en place de voir-clavier.
...
const debord = async (pg) => pg.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
{
  ...
  console.log('memoire visible:', await pg.locator('text=Toujours donner les visiteurs').count(), 'debord', await debord(pg));
  await ctx.close();
}
await b.close();
