...
await page.goto('http://localhost:4711/vendor/products/bulk',{waitUntil:'domcontentloaded'});
await page.waitForTimeout(2500);
const photos=readdirSync(join(DIST,'demo-products')).filter(f=>f.endsWith('.jpg')).slice(0,5).map(f=>join(DIST,'demo-products',f));
await page.locator('input[type=file]').first().setInputFiles(photos);
await page.waitForTimeout(7000);
const nameInputs=page.locator('.space-y-2 input[placeholder="Nom du produit"]');
// Un nom déjà tapé à la main ne doit PAS être écrasé
await nameInputs.nth(1).fill('Robe wax spéciale');
await page.locator('section input').first().fill('Pyjama fille');
await page.locator('section input[type=number]').first().fill('3500');
await page.locator('button',{hasText:'Appliquer à tous les articles'}).click();
await page.waitForTimeout(1200);
console.log('Noms  :', await nameInputs.evaluateAll(e=>e.map(x=>x.value)));
...
console.log('Bouton:', (await btn.textContent())?.trim());
await btn.click(); await page.waitForTimeout(2000);
console.log('En base:', Array.isArray(inserted)?inserted.map(x=>x.name):null);
