...
| A5 | Ils connaissent les outils (Tableau, Notion, Excel, Power BI) et s'en servent | ✅ GitHub, boutique Finjaro, recherche sur Internet ; ✅ 23/09 : fichiers Excel, CSV, PDF déposés dans un salon et lus par les agents, et un tableau rendu en vrai fichier Excel (B2) ; ⏸ Google Sheets, Drive, Notion en direct (projet Google, jetons : Beau) |
...
| B2 | Connectés à **Excel** | ✅ 23/09 (staging), essayé en vrai : « Fichier (Excel, CSV, PDF) » dans le menu + ; le serveur lit chaque feuille ; l'agent rend un .xlsx avec des formules (marge, taux en %, total) en 14 secondes, sans recopier un chiffre à la main |
...
| D7 | Une nouvelle entreprise trouve déjà quelque chose de très spécialisé | ✅ 23/09 : « + Agent » cherche dans le catalogue (3 737 postes) et remplit le poste, le mandat et le département |
...
| # | Ce que Beau veut | État vérifié |
| --- | --- | --- |
| E1 | À l'arrivée : le profil de l'entreprise (taille, secteur, **outils utilisés** : Excel, Notion…) | ✅ taille, secteur, « qui es-tu », « ce que tu veux obtenir » ; ✅ 23/09 : « Tes outils de travail » à la fondation (Excel, WhatsApp, Notion, Power BI…) |
| E2 | Les agents s'adaptent à ce profil | ✅ le projet est lu par chaque agent, outils compris : il rend ses livrables dans ces formats |
| E3 | Modifier par agent : personnalité, **consignes de sécurité**, **restrictions d'accès aux données** | ✅ personnalité ; ✅ 23/09 : « Ce qu'il ne fait jamais » et « Ce qu'il peut lire » (chiffres, boutique, Internet, dépôt de code, documents), par agent, dans sa fiche ; les réponses, le travail du matin et les réunions s'y tiennent |
| E4 | (transparence) Voir exactement la consigne qu'un agent reçoit | ✅ 23/09 : dans sa fiche, « Ce qu'il reçoit avant chaque réponse », partie par partie, avec le vrai contenu |
...
| --- | --- | --- |
| I1 | « Connecter mes agents avec Excel » | ✅ 23/09 : déposer un Excel ou un CSV dans un salon ; chaque feuille est lue (300 lignes au plus par feuille) et gardée avec le message |
| I2 | « Capable d'ouvrir Excel et de le modifier » | ✅ 23/09 : l'agent rend le fichier modifié en .xlsx (colonnes ajoutées, formules gardées, taux en pourcentage), et dit ce qu'il a changé et ce qu'il faut vérifier ; essayé en vrai sur un tableau de ventes |
| I3 | « Agents IA par service : vous avez 20 personnes au service X ? On vous envoie les agents qui font ceci et ceci » | ✅ 23/09 (staging) : « Renforcer » à côté de « + Agent » ; on décrit le service, Legion propose 2 à 5 agents (ce qu'il prend, ce qu'il ne fait jamais, à qui il passe la main, ce qui reste aux humains) ; on coche, ils arrivent avec leurs premières tâches. Essayé sur un service client de 20 personnes |
| I4 | « Une boîte d'intérim de service client, donc il y aura plusieurs entreprises » | ✅ 23/09 : le modèle « Agence d'intérim IA » est au catalogue (54 postes, 13 départements, dont Supervision et Qualité, Juridique et Conformité) ; ⏸ prix et avis juridique avant le service client intérim (voir LEGION-INTERIM-ETUDE.md) |
| I5 | « Auditum, agent IA expert en audit : si quelqu'un veut, on le branche au truc de l'entreprise et il fait — agent intérim » | ✅ 23/09 : « Un expert pour une mission » (audit, clôture, paie, recouvrement, données personnelles, contrats, étude de marché…) ; une date de fin fait de lui un intérimaire qui s'éteint seul le lendemain et le dit (vérifié) ; essayé en vrai avec une experte en relecture de contrats |
...
| J2 | (vidéo « service client automatisé ») Le Drive de l'entreprise est branché : chaque document déposé entre dans une base, **l'agent est toujours à jour** ; la boîte mail est branchée ; un **tri** dit si l'e-mail est une demande client ou autre chose ; « autre » → l'agent ne fait **rien** ; demande client → l'agent (sa consigne : chez qui il travaille, sa mission ; un modèle pas cher) cherche dans la base, **étiquette** l'e-mail « service client » pour que l'entreprise vérifie, et **répond** | ✅ 23/09 (staging), essayé en vrai : **Documents de l'entreprise** sur l'accueil de Legion — on dépose un PDF, un Excel, un CSV ou un texte (ou on colle une FAQ) ; il est découpé et indexé par le sens ; un agent qui a le droit de les lire répond à partir d'eux, cite le titre, et dit quand il ne trouve pas (la relecture les connaît aussi : un prix de la FAQ n'est plus retiré comme « inventé »). ✅ **Trier une demande client** : on colle un message reçu ; Legion dit « demande client », « autre » ou « rien à faire », l'urgence, et prépare le brouillon depuis les documents — sans remboursement, prix ni délai qui n'y serait pas écrit, avec « Réponse préparée avec l'aide d'un assistant IA » et ce qui manque aux documents. ⏸ brancher la vraie boîte mail et répondre seul : Gmail (projet Google de Beau) ou une adresse de réception Cloudflare (jeton Cloudflare de Beau) |
