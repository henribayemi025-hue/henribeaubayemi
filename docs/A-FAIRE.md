...
| 25/09 | Beau, « Audit complet Finjaro Léo » (version ordinateur ; la version téléphone suivra) + capture de la tour de contrôle d'une entreprise « test for salle de marché » : (1) page d'accueil : il ne l'aime pas, veut en fond une photo ou une vidéo de la cité avec des agents qui marchent et travaillent (Gemini ou Seedance) ; ajouter « Continue with Finjaro » à côté de Google et Apple ; (2) nouveau compte → Overview → My companies : il doit pouvoir entrer dans son entreprise ; « Legion » encore partout (« Comment Legion travaille chez toi ») alors que c'est Léo ; « New company » apparaît 3 fois sur la page ; la partie visuels n'est pas belle ; toujours revérifier ses erreurs, et revérifier encore ; (3) « Your companies and the teams working in them » : que ça défile comme sur Finjaro, images des agents au travail ; deux palettes au choix (la sombre actuelle et le blanc de Finjaro) ; (4) « Found a company » : il n'aime pas les emoji des cabinets → à gauche des photos d'agents qui défilent (agence de croissance, laboratoire…) ; il a choisi « salle de marché » ; les « 71 roles, each mandates a department » doivent être cliquables : voir, ajouter, définir un rôle par un prompt, déposer un fichier, et un agent déjà présent pour aider à trouver et répartir les rôles et départements ; « What it is » modifiable (pour lui seul) ; « Found it » : bouton dynamique et beau ; (5) une fois entré (tour de contrôle) : ça doit défiler, beau design, des formes qui ne « font pas IA » — « là ça fait trop app IA, pas une app comme Google » | en attente de « go Claude lis » : rien traité ni répondu |
| 25/09 | Beau (suite de l'audit, 4 captures : ordre « how are you planning to work » à Direction, deux micros superposés en bas du salon, colonne de salons « # » tous pareils, salon « Gestion » vide avec Info / Question / Proposal / To decide / Task en emoji) : rendre tout dynamique — le lion et les emoji (ex. « building ») à remplacer par de vrais visuels qui bougent au survol ; à la création, les agents doivent déjà avoir leur photo réelle et choisir leur nom ; la réponse de Direction propose 3 livrables en Excel ou Power BI — un agent peut-il vraiment envoyer du Power BI ? ; les salons « ça va », à rendre encore plus beaux ; deux icônes d'audio et plusieurs icônes qu'il ne comprend pas ; la génération Gemini dans « Gestion » ne donne qu'une petite image — tenir avec 25, 100, 10 000 agents ; il est allé dans la ville (« tu travailles toujours dessus, je ne te mets pas la pression ») | en attente de « go Claude lis » : rien traité ni répondu · mer, bateau et plongée terminés en parallèle (ligne du jour) |
| 25/09 | Beau (compte de test Léo neuf, entreprise « test for salle de marché ») : l'Atelier dit « Aucun modèle disponible : aucune clé dans le Worker, et le relais Supabase n'en propose aucun » ; « même les commentaires sont en français alors que j'ai choisi l'anglais » | cause trouvée : le Worker de l'atelier accepte tout propriétaire d'entreprise depuis le 25/09, mais la fonction commune `atelier-modele` n'accepte encore que le propriétaire de l'entreprise Finjaro → pour un nouveau compte, le relais refuse, et le message affiché est trompeur. ✅ 25/09 (« oui, ouvre le relais ») : `atelier-modele` v5 déployée — même règle que le Worker (propriétaire de n'importe quelle entreprise Léo), plafond 1 $/jour/personne gardé (fonction commune staging + production ; Finjaro Accounting ne l'utilise pas) · à confirmer par l'essai de Beau avec son compte de test · messages de l'atelier en français quelle que soit la langue : à traduire |
