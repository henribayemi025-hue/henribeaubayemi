...
| 24/09 | → le code V0 est prêt (pas poussé) : **les étapes exactes sont dans `docs/ATELIER-V0.md`** (offre payante, Worker `finjaro-atelier` branché sur le dossier `atelier` et la branche `staging`, secrets `DEEPSEEK_API_KEY` et facultatifs `KIMI_API_KEY`, `GEMINI_API_KEY`) | l'atelier reste « pas encore en ligne » tant que ce n'est pas fait |
| 24/09 | **Appliquer la migration 0200** (`atelier_*`, additive, rien d'existant touché) — seulement quand Beau le décide | la trace durable de l'atelier (sans elle, il marche quand même) |
